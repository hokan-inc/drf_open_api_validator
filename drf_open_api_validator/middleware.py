...
class DjangoOpenAPIRequestValidationMiddleware:
    ...
    def __call__(self, request: HttpRequest):
        ...
        if openapi_request:
            try:
                openapi_result_path = self.path_finder.find(openapi_request)
                ...
                logger.warning("Invalid path: unknown error", exc_info=True)
                if self.strict_path:
                    raise
            else:
                if openapi_result.errors:
                    logger.warning("Invalid request: %s", openapi_result.errors)
        ...
        if openapi_request and openapi_result_path:
            try:
                ...
                logger.warning("Invalid response: unknown error", exc_info=True)
                if self.strict_response_schema:
                    raise
